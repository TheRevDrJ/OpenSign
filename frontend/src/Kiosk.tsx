...
export default function Kiosk() {
  ...
  useEffect(() => {
    let alive = true
    // First reload token seen by this page; a different one later means the admin
    // pressed "Reload displays" or the server restarted (possibly with new code).
    let seenToken: number | undefined
    const load = async () => {
      const c = await getConfig()
      if (!alive) return
      if (c.reloadToken !== undefined) {
        if (seenToken === undefined) seenToken = c.reloadToken
        else if (c.reloadToken !== seenToken) {
