...
Optional overlays ride on top of whatever mode is running: **clock**, **calendar**,
**countdown**, **giving** (your QR code) and **verse of the day**. Drag each onto a snap
grid — set to Landscape or Portrait to match your screen — with a glass look; add a
widget by dragging it on, remove it with a right-click, and pick S/M/L/XL.
...
**Reload displays** makes every connected screen reload itself within a few seconds.

### Verse of the day — translations

The verse comes from a curated list of well-loved passages (selection from
[OpenBible.info](https://www.openbible.info/), CC BY 4.0), and the same verse shows on
every screen each day. The **Berean Standard Bible** is built in and works offline.

To show another translation (NIV, NLT, ESV, a Spanish or Korean Bible, …), get a free
key from **[API.Bible](https://api.bible/)**: sign up for the Starter plan, pick up to
three copyrighted translations, then paste the key into **Admin → Widgets → Verse** and
choose a language and translation. Your key also unlocks API.Bible's free translations.
The server fetches each day's verse itself; nothing copyrighted is stored in this
repository, and fetched verses are deleted after 30 days as API.Bible's terms require.
If the key is missing or the internet is down, the widget shows the BSB instead.
...
setup checklist — notably: **the display box must reboot nightly** (a kiosk left up
for days can stop reaching the server until it restarts), and disable sleep/power-saving.
